...
- 提供项目级 AI 助手底部入口与居中浮动工作区，桌面历史侧栏可折叠、切换和行内重命名，窄屏历史使用抽屉。助手支持智能组合、已选证据、当前文档、当前项目和通用知识五档范围；项目来源由服务端重新校验，通用知识与本地模拟显式标记，联网关闭。对话保存在项目数据库中且可逐条或整段删除，历史默认不进入下一次请求；转存备忘录或创建待复核风险草稿必须人工确认。
...
PDF 归档件的直接生成、字体嵌入、导出审计和逐页渲染结果记录在
[ADR-0016](docs/adr/0016-pdf-archive-export.md) 与
[迭代八 PDF 归档导出验收](docs/acceptance/iteration-8-pdf-export-2026-09-23.md)。当前 P0、F01 至 F12、P01 至 P07 的完成状态和后续门禁记录在
[MVP P0 追踪矩阵](docs/acceptance/mvp-p0-traceability-2026-09-23.md)。
...
[ADR-0023](docs/adr/0023-project-assistant-context-and-history.md)记录项目助手的范围、对话历史、引用校验和人工写入边界；[系统提示词 v1](docs/prompts/assistant-system-v1.md)用于业务确认。
[迭代十九项目级 AI 助手验收](docs/acceptance/iteration-19-project-assistant-2026-09-24.md)记录桌面底部入口、五档范围、项目本地对话、人工写入确认、自动化结果和多视口视觉回归。
...
管理层沟通材料、四类成果固化边界和独立证据包索引记录在
[ADR-0021](docs/adr/0021-management-materials-and-evidence-package.md)，实现与成品渲染结果记录在
[迭代十三管理层材料与证据包验收](docs/acceptance/iteration-13-management-materials-evidence-package-2026-09-24.md)。
