...
- 输出区可从“已核实”或“已关闭”风险生成格式无关的不可变快照，再本地确定性生成风险清单、资料清单和访谈提纲。三类内容共用可编辑草稿、版本历史和最终固化门禁；当前已固化草稿可按仓库中文模板生成 Excel 风险清单、Word 工作成果包和 PDF 归档件。PDF 直接读取固化草稿并嵌入 Windows 中文字体，不从 Word 反向转换。每次导出均记录格式、模板、草稿版本、源快照和文件 SHA-256，历史文件只增不覆盖。
