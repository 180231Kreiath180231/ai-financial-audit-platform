...
- 输出区可从“已核实”或“已关闭”风险生成格式无关的不可变快照，再本地确定性生成风险清单、资料清单和访谈提纲。三类内容共用可编辑草稿、版本历史和最终固化门禁；当前已固化草稿可按仓库中文模板生成含风险清单、规则计算和证据索引的 Excel 文件，每次导出均记录模板、草稿版本、源快照和文件 SHA-256，历史文件只增不覆盖。

真实审计资料的外部视觉服务、真实文本模型业务调用、统计异常分析、自动风险分级以及 Word、PDF 文件导出仍未启用，不代表功能已实现。PaddleOCR 当前只完成合成数据专项接入；Excel 导出完全在本地生成，不调用外部模型。
